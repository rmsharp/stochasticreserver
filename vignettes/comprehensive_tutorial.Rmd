...
$$A_{ij} \sim N(\mu_{ij}, \sigma^2_{ij})$$

where the variance depends on the expected value:

$$\sigma^2_{ij} = e^{\kappa - w_i} \cdot (\mu_{ij}^2)^p$$

Here $\kappa$ is a proportionality constant, $w_i = \log(d_i)$ where $d_i$ is
the exposure count for accident year $i$, and $p$ is a power parameter
controlling how variance scales with the mean.
